...
###Files to read and write should inserted instead of "...\\output\\1_geopoints.tsv", etc
...
data_continent <- bind_rows(data_continent, data_ocean)
...
for (i in seq(1 : length(data_group))) {
	...
	assign(row_number_i_not, data_continent |> filter(continent != data_group[i] & continent != "Continentless") |> pull(row_number))
}
...
umap_start <- Sys.time()
continents_umap <- umap( continent_distance, preserve.seed = TRUE, n_neighbors = n )
umap_end <- Sys.time()
umap_elapsed <- difftime(umap_end, umap_start, units = "secs")
umap_time[i,1] <- umap_elapsed
umap_time[i,2] <- n
#Get the centered UMAP coordinates 
data_continent$umap1_centered <- scale(continents_umap$layout[,1])
data_continent$umap2_centered <- scale(continents_umap$layout[,2])
#Plot the results
scatter_plot <- ggplot(data_continent |> filter(continent != "Continentless"), aes(x=umap1_centered, y=umap2_centered, color=continent)) +
	geom_point(size = .2) +
	...
	coord_fixed() +
	theme_classic()
ggsave(str_glue(".../output/pics/covered_fraq3/UMAP_scatter_n-{n}.png"), scatter_plot, device = "png",
...
write(total_ratio_inside, str_glue(".../output/distance_ratio_covered_fraq3/total_ratio_inside_n-{n}.tsv"), ncolumns = 1)
...
#Save the results
write(total_ratio_inside_not, str_glue(".../output/distance_ratio_covered_fraq3/total_ratio_inside_not_n-{n}.tsv"), ncolumns = 1)
...
write_tsv(umap_time, ".../output/umap_covered_fraq3_time.tsv")